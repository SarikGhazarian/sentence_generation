...
class LM1BDataset(object):
  ...
  def __init__(self, filepattern, vocab):
    ...
    #tf.logging.info('Found %d shards at %s', len(self._all_shards), sentence)
    

  def _load_random_shard(self):
    """Randomly select a file and read it."""
    #return self._load_shard(random.choice(self._all_shards))
    return self._load_shard(self._all_shards)
  ...
  def _load_shard(self, shard_name):
    ...
    sentence = shard_name
    chars_ids = [self.vocab.encode_chars(sentence)] # list of char ids of words in each sentence (a list of arrays (list of lists))
    ids = [self.vocab.encode(sentence)] #list of words ids in each sentence (a list of lists)    
    #chars_ids = [self.vocab.encode_chars(sentence) for sentence in sentences] # list of char ids of words in each sentence (a list of arrays (list of lists))
    #ids = [self.vocab.encode(sentence) for sentence in sentences] #list of words ids in each sentence (a list of lists)
    global_word_ids = []
    current_idx = 0
    for word_ids in ids:
